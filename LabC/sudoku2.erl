...
par_solve_refined0(M,0) -> solve_refined(M);
par_solve_refined0(M,N) ->     
    case solved(M) of
	true ->
	    M;
	false ->
...
par_solve_one(MS,N) ->
    ...
    Ref = make_ref(),
    [spawn(fun() -> 
	     case catch par_solve_refined(M,N-1) of
		 {'EXIT',no_solution} ->
     ...
     || M <- MS],
    receive_solution(Ref,lists:sum([1 ||_ <- MS])).
